 ...
 switch(typeDist, 
     "Continuous"=
      ...
      "Mixed"=
          ...
          mod <- if (is.null(freq)) 
                  {  # no frequencies
                  if (is.null(data)) gamlss(y~1, family=fname,  ...)
                  else gamlss(y~1, family=fname, data=data, ...)
                  }
               else
                  { # with frequencies
                 # .freq <-freq
                   if (is.null(data))  gamlss(y~1, weights=freq,  family=fname, ...)
                   else gamlss(y~1, weights=freq,  family=fname, data=data, ...)
                  } 
                         mod$call$family <- eval(as.expression(fname)) 
                   mod$call$formula[[2]] <- substitute(y)
